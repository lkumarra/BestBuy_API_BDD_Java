package demo.bestbuy.com.helpers.stores;

import java.util.*;
import java.util.stream.Collectors;

import demo.bestbuy.com.data.dbKeys.DBkeys.StoresTableKeys;
import demo.bestbuy.com.helpers.apihelper.StringHelper;
import demo.bestbuy.com.helpers.dbhelpers.DBHelpers;
import demo.bestbuy.com.modals.stores.StoresModal;
...
public final class StoresDBHelper {
    ...
    public static List<StoresData> getStoresFromDb() {
        ...
        Map<Integer, StoresData> storesMap = dataTable.stream().collect(Collectors.toMap(
                row -> Integer.parseInt(row.get(StoresTableKeys.STORESID)), // Key: Store ID
                row -> {
                    StoresData store = new StoresData();
                    store.setId(Integer.parseInt(row.get(StoresTableKeys.STORESID)));
                    store.setName(row.get(StoresTableKeys.STORESNAME));
                    ...
                    store.setCreatedAt(formatDateString(row.get(StoresTableKeys.STORESCREATEDAT)));
                    store.setUpdatedAt(formatDateString(row.get(StoresTableKeys.STORESUPDATEDAT)));
                    return store;
                },
    ...
    public static List<StoresServices> getStoresServices(List<HashMap<String, String>> dataTable) {
        ...
        return dataTable.stream().map(row -> {
            ...
            try {
                storesServices.setStoreId(Integer.parseInt(row.get(StoresTableKeys.STORESID)));
                storesServices.setServiceId(Integer.parseInt(row.get(StoresTableKeys.SERVICESID)));
                storesServices.setCreatedAt(formatDateString(row.get(StoresTableKeys.STORESSERVICECREATEDAT)));
                storesServices.setUpdatedAt(formatDateString(row.get(StoresTableKeys.STORESSERVICEUPDATEDAT)));
            } catch (Exception e) {
    ...
    public static List<Services> getServices(List<HashMap<String, String>> dataTable) {
        ...
        return dataTable.stream().map(row -> {
                    ...
                    int storeId = Integer.parseInt(row.get(StoresTableKeys.STORESID));
                    int serviceId = Integer.parseInt(row.get(StoresTableKeys.SERVICESID));
                    String key = storeId + ":" + serviceId;

                    services.setId(serviceId);
                    services.setName(row.get(StoresTableKeys.SERVICESNAME));
                    services.setCreatedAt(formatDateString(row.get(StoresTableKeys.SERVICESCREATEDAT)));
                    services.setUpdatedAt(formatDateString(row.get(StoresTableKeys.SERVICEUPDATEDAT)));
